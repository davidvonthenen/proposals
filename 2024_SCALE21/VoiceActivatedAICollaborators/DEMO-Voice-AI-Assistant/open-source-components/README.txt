...
9. make
10. In a new console, run:
    ./server -m models/7B/llama-2-7b-chat.Q4_K_M.gguf -c 2048

Go back up to <ROOT>:
11. python open-source-llama2.py


PocketSphinx STT + Open Llama + pyttx3
------------------------
Steps to run this configuration starting from the same directory as this README.txt.
Will call this repo <ROOT>.
1. [recommended but not required] Create a Python Environment using something like Conda
2. pip install -r requirements.txt
3. cp ./downloads/hf_download_open_llama.py ./
4. python open-source-open-llama.py
