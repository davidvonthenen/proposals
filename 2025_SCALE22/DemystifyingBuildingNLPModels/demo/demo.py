...
print("Run the QUESTION model with REST API from the ../question folder!")
print("Step 2: Done!")
print("Run the NER model with REST API from the ../question folder!")
print("Step 3: Done!")
    ...
    try:
        completion = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Your name is Elizabeth. You are a helpful assistant. Always limit your responses to using bullet points of the top 3 facts about a subject, but limit the response to 100 words or less. Do not use any abbreviations for anything.",
                },
                {
                    "role": "user",
...
def main():
    try:
        ...
        if not dg_speak_connection.start(speak_options):
            print("Failed to start speak connection")
            return
        ...
        if not dg_listen_connection.start(listen_options):
            print("Failed to start listen connection")
            return
