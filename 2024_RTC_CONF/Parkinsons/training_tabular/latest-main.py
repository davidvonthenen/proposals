...
def compute_features_with_resilient_landmarks(df):
    ...
    if all(
        [
            ...
            for landmark in ["RIGHT_SHOULDER", "RIGHT_HIP", "RIGHT_KNEE"]
        ]
    ):
        df_pivot["RIGHT_LEAN_ANGLE"] = calculate_angle(
            df_pivot, "RIGHT_SHOULDER", "RIGHT_HIP", "RIGHT_KNEE"
        )
    if all(
        [
            f"x_{landmark}" in df_pivot.columns
        ...
        df_pivot["LEFT_LEAN_ANGLE"] = calculate_angle(
            df_pivot, "LEFT_SHOULDER", "LEFT_HIP", "LEFT_KNEE"
        )
    ...
    if all(
        [
            f"x_{landmark}" in df_pivot.columns
            for landmark in ["RIGHT_ANKLE", "RIGHT_HIP", "LEFT_ANKLE"]
        ]
    ):
        df_pivot["RIGHT_STEP_ANGLE"] = calculate_angle(
            df_pivot, "RIGHT_ANKLE", "RIGHT_HIP", "LEFT_ANKLE"
        )
    if all(
        [
            f"x_{landmark}" in df_pivot.columns
            for landmark in ["LEFT_ANKLE", "LEFT_HIP", "RIGHT_ANKLE"]
        ]
    ):
        df_pivot["LEFT_STEP_ANGLE"] = calculate_angle(
            df_pivot, "LEFT_ANKLE", "LEFT_HIP", "RIGHT_ANKLE"
        )
